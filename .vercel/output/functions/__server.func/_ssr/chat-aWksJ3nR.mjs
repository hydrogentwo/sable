...
//#region node_modules/.nitro/vite/services/ssr/assets/chat-aWksJ3nR.js
function systemPrompt(input) {
	const runtime = input.artifact ?? (input.accelerator === "tpu" ? "TPU path queued — LiteRT INT4 AOT not compiled yet on this host" : input.accelerator === "gpu" ? "GPU path — LiteRT INT4 / WebGPU" : "CPU path — GGUF Q4_K_M");
...
var runSableTurn_createServerFn_handler = createServerRpc({
	id: "87a9557e2f50c40b37faac98b149539793fff8e2adc5a252efe4204a1ad3611c",
	name: "runSableTurn",
	filename: "src/lib/server/chat.ts"
}, (opts) => runSableTurn.__executeServer(opts));
var runSableTurn = createServerFn({ method: "POST" }).validator((input) => input).handler(runSableTurn_createServerFn_handler, async ({ data }) => {
	const apiKey = process.env.XAI_API_KEY;
	if (!apiKey) return {
		ok: false,
		error: "On-device runtime is not available in this environment."
	};
	const recent = data.messages.slice(-12);
	const body = {
		model: "grok-4.5",
		temperature: data.think ? 1 : .7,
		top_p: .95,
		max_tokens: 700,
		messages: [{
			role: "system",
	...
	return {
		ok: true,
		...splitThink((await res.json()).choices?.[0]?.message?.content ?? "")
	};
});
//#endregion
export { runSableTurn_createServerFn_handler };
