...
function parseGithub(input: string): { owner: string; repo: string; file?: string; ref?: string } | null {
  ...
  try {
    ...
    if (parts[2] === "blob" && parts.length >= 5) {
      return { owner, repo, ref: parts[3], file: parts.slice(4).join("/") };
    }
...
function guessCategory(text: string): SkillCategory {
...
async function fetchRaw(url: string): Promise<string | null> {
  try {
    const res = await fetch(url, {
      headers: { Accept: "text/plain", "User-Agent": "sable-skill-grab" },
    });
...
export const grabSkillFromGithub = createServerFn({ method: "POST" })
  .validator((input: { url: string }) => input)
  .handler(async ({ data }) => {
    const parsed = parseGithub(data.url);
    if (!parsed) {
      return { ok: false as const, error: "Need a GitHub URL or owner/repo." };
    }
    const refs = parsed.ref ? [parsed.ref, "HEAD"] : ["HEAD", "main", "master"];
    ...
    outer: for (const ref of refs) {
      for (const file of files) {
        const url = `https://raw.githubusercontent.com/${parsed.owner}/${parsed.repo}/${ref}/${file}`;
    ...
    const description =
      fm.description?.trim() ||
      fm.body.split("\n").find((l) => l.trim() && !l.startsWith("#") && !l.startsWith("["))?.slice(0, 220) ||
      `Grabbed skill from ${repo}`;
    const body = fm.body.slice(0, 8000);
    const skill: Skill = {
    ...
    return { ok: true as const, skill, file: used };
  });
